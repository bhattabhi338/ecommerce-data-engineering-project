...
source_layer = 'bronze'
target_layer = 'silver'
source_table = target_table = 'd_products'
...
try:
    ...
    if delta_df.isEmpty():
        ...
        #Fetch the latest records to be inserted
        latest_records = spark.sql(f"select * from temp_source s where not exists (select 1 from {catalog_name}.{target_layer}.{target_table} t where {join_condition} and t.dw_eff_end_dt is null)")


        #Insert the latest records into our silver table
        latest_records.createOrReplaceTempView('insert_records')

        current_time = spark.sql("select current_timestamp()").collect()[0][0]

        insert_query = f"INSERT INTO {catalog_name}.{target_layer}.{target_table} ({insert_cols}) select {insert_condition}, '{current_time}', '{current_time}', '{master_id}', null, '{last_load_end}', null from insert_records s"
